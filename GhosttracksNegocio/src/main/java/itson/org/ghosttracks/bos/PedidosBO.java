...
import itson.org.ghosttracks.persistencia.PedidosDAO; 
...
import itson.org.ghosttracks.utilidades.GeneradorFolios;
...
public class PedidosBO implements IPedidosBO {
    ...
    public PedidoDTO generarPedido(NuevoPedidoDTO nuevoPedido) throws NegocioException {
        ...
        try {
            ...
            LOGGER.log(Level.INFO, "Pedido y Pago registrados con éxito. ID: {0}, Folio: {1}", 
                    new Object[]{guardado.getIdPedido(), guardado.getFolio()});

            PedidoDTO respuesta = PedidoMapper.toDTO(guardado);

            if (respuesta != null) {
                respuesta.setCliente(nuevoPedido.getCliente());
                respuesta.setCarrito(nuevoPedido.getCarrito()); 
                respuesta.setContacto(nuevoPedido.getContacto());
                respuesta.setDireccionEntrega(nuevoPedido.getDireccionEntrega());
                respuesta.setCostoEnvio(guardado.getCostoEnvio() != null ? guardado.getCostoEnvio() : 0.0);
            }
    ...
    private Pedido construirEntidadPedido(NuevoPedidoDTO nuevoPedido) {
        ...
        entidad.setCostoEnvio(0.0); 

        entidad.setFolio(GeneradorFolios.generarFolioPedido());
        ...
        if (nuevoPedido.getCliente() != null) {
            entidad.setIdCliente(nuevoPedido.getCliente().getIdCliente());
        }
    ...
    private Paquete construirPaquete(String idPedido, Double peso, Double largo, Double ancho, Double alto) throws Exception {
